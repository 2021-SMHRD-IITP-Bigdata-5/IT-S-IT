...
public class Board_DAO {
	public int insert(String id, String title, String contents)
	{
		...
		try {
			dao.getConn();
			String sql = "INSERT INTO tbl_daily_board (article_subject, article_content, reg_date, article_likes, mem_id) VALUES (?, ?,sysdate, 0, ?)";
	...
	public ArrayList<Board_DTO> select(String mem_id) {
		...
		try {
			dao.getConn();
			String sql = "Select article_subject, article_content, article_file, reg_date from tbl_daily_board where id = ?";
			dao.psmt = dao.conn.prepareStatement(sql);
